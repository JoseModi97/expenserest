...
const CategoriesListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/categories/view/${data.category_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/categories/edit/${data.category_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
	...
	function CategoryIdTemplate(data){
		if(data){
			return (
				<Link to={`/categories/view/${data.category_id}`}> { data.category_id }</Link>
			);
		}
	}
	...
	return (
<main id="CategoriesListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('categories')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/categories/add`}>
                        <Button label={$t('addNewCategory')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="category_name" header={$t('categoryName')}   ></Column>
...
CategoriesListPage.defaultProps = {
	primaryKey: 'category_id',
	pageName: 'categories',
	apiPath: 'categories/index',
