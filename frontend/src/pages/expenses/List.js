...
const ExpensesListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/expenses/view/${data.expense_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/expenses/edit/${data.expense_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
	...
	function ExpenseIdTemplate(data){
		if(data){
			return (
				<Link to={`/expenses/view/${data.expense_id}`}> { data.expense_id }</Link>
			);
		}
	}
	function UserIdTemplate(data){
		if(data){
			return (
				<>{data.user_id && <Button className="p-button-text" icon="pi pi-eye" label={$t('users')} onClick={() => app.openPageDialog(<UsersViewPage isSubPage apiPath={`/users/view/${data.user_id}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	function CategoryIdTemplate(data){
		if(data){
			return (
				<>{data.category_id && <Button className="p-button-text" icon="pi pi-eye" label={$t('categories')} onClick={() => app.openPageDialog(<CategoriesViewPage isSubPage apiPath={`/categories/view/${data.category_id}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	return (
<main id="ExpensesListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('expenses')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/expenses/add`}>
                        <Button label={$t('addNewExpens')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="expense_id" header={$t('expenseId')} body={ExpenseIdTemplate}  ></Column>
                                    <Column  field="user_id" header={$t('userId')} body={UserIdTemplate}  ></Column>
                                    <Column  field="category_id" header={$t('categoryId')} body={CategoryIdTemplate}  ></Column>
                                    ...
                                    <Column  field="createdat" header={$t('createdat')}   ></Column>
                                    <Column  field="updatedat" header={$t('updatedat')}   ></Column>
...
ExpensesListPage.defaultProps = {
	primaryKey: 'expense_id',
	pageName: 'expenses',
	apiPath: 'expenses/index',
