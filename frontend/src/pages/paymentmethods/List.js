...
const PaymentmethodsListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: $t('search'),
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/paymentmethods/view/${data.payment_method_id}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/paymentmethods/edit/${data.payment_method_id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.payment_method_id) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function PaymentMethodIdTemplate(data){
		if(data){
			return (
				<Link to={`/paymentmethods/view/${data.payment_method_id}`}> { data.payment_method_id }</Link>
			);
		}
	}
	...
	return (
<main id="PaymentmethodsListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('paymentmethods')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/paymentmethods/add`}>
                        <Button label={$t('addNewPaymentmethod')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="payment_method_id" header={$t('paymentMethodId')} body={PaymentMethodIdTemplate}  ></Column>
                                    <Column  field="user_id" header={$t('userId')} body={UserIdTemplate}  ></Column>
                                    <Column  field="payment_method_name" header={$t('paymentMethodName')}   ></Column>
...
PaymentmethodsListPage.defaultProps = {
	primaryKey: 'payment_method_id',
	pageName: 'paymentmethods',
	apiPath: 'paymentmethods/index',
	routeName: 'paymentmethodslist',
